import { isSameDay, isDateDisabled, startOfDay } from '../utils/date';
import { parseDate, formatDate } from '../utils/locale';
import { yearPageStart } from '../utils/calendar';
import type { ViewMode, DatePickerConfig } from '../types';
...
export function datePickerReducer(
  ...
  switch (action.type) {
    ...
    case 'CLOSE':
      return { ...state, open: false, openSource: null, view: 'day', hoverDate: null };

    case 'TOGGLE':
      if (state.open) return { ...state, open: false, openSource: null, view: 'day', hoverDate: null };
    ...
    case 'SELECT_DATE': {
      ...
      if (config.mode === 'range') {
        ...
        const [start, end] =
          d.getTime() >= state.rangeStart.getTime()
            ? [state.rangeStart, d]
            : [d, state.rangeStart];
        return {
          ...state,
    ...
    case 'HOVER_DATE':
      return { ...state, hoverDate: action.date ? startOfDay(action.date) : null };
