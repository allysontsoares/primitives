...
function AccordionItem({ className, ...props }: AccordionPrimitive.Item.Props) {
  return (
    <AccordionPrimitive.Item
      ...
      className={cn("not-last:border-b data-open:bg-oklch(0.97 0 0)/50 dark:data-open:bg-oklch(0.269 0 0)/50", className)}
      {...props}
    />
  )
}
...
function AccordionContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "h-(--accordion-panel-height) pt-0 pb-4 data-ending-style:h-0 data-starting-style:h-0 [&_a]:underline [&_a]:underline-offset-3 [&_a]:hover:text-oklch(0.145 0 0) [&_p:not(:last-child)]:mb-4 dark:[&_a]:hover:text-oklch(0.985 0 0)",
          className
        )}
      >
        {children}
