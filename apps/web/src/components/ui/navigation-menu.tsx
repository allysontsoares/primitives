  ...
  return (
    <NavigationMenuPrimitive.Portal>
      <NavigationMenuPrimitive.Positioner
        ...
        {...props}
      >
        <NavigationMenuPrimitive.Popup className="data-[ending-style]:easing-[ease] xs:w-(--popup-width) relative h-(--popup-height) w-(--popup-width) origin-(--transform-origin) rounded-3xl bg-oklch(1 0 0) text-oklch(0.145 0 0) shadow-lg ring-1 ring-oklch(0.145 0 0)/5 transition-[opacity,transform,width,height,scale,translate] duration-[0.35s] ease-[cubic-bezier(0.22,1,0.36,1)] outline-none data-ending-style:scale-90 data-ending-style:opacity-0 data-ending-style:duration-150 data-starting-style:scale-90 data-starting-style:opacity-0 dark:ring-oklch(0.145 0 0)/10 dark:bg-oklch(0.205 0 0) dark:text-oklch(0.985 0 0) dark:ring-oklch(0.985 0 0)/5 dark:dark:ring-oklch(0.985 0 0)/10">
...
function NavigationMenuLink({ className, ...props }: NavigationMenuPrimitive.Link.Props) {
  return (
    <NavigationMenuPrimitive.Link
      ...
      className={cn(
        "flex items-center gap-2 rounded-2xl px-2.5 py-1.5 text-sm font-medium transition-all outline-none hover:bg-oklch(0.97 0 0) focus:bg-oklch(0.97 0 0) focus-visible:ring-3 focus-visible:ring-oklch(0.708 0 0)/30 focus-visible:outline-1 in-data-[slot=navigation-menu-content]:w-full in-data-[slot=navigation-menu-content]:rounded-xl in-data-[slot=navigation-menu-content]:p-2 in-data-[slot=navigation-menu-content]:font-normal data-[active=true]:bg-oklch(0.97 0 0)/50 data-[active=true]:hover:bg-oklch(0.97 0 0) data-[active=true]:focus:bg-oklch(0.97 0 0) [&_svg:not([class*='size-'])]:size-4 dark:hover:bg-oklch(0.269 0 0) dark:focus:bg-oklch(0.269 0 0) dark:focus-visible:ring-oklch(0.556 0 0)/30 dark:data-[active=true]:bg-oklch(0.269 0 0)/50 dark:data-[active=true]:hover:bg-oklch(0.269 0 0) dark:data-[active=true]:focus:bg-oklch(0.269 0 0)",
        className,
      )}
      {...props}
    />
