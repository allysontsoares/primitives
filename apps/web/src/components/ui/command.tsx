...
function Command({ className, ...props }: React.ComponentProps<typeof CommandPrimitive>) {
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "flex size-full flex-col overflow-hidden rounded-3xl bg-oklch(1 0 0) p-1 text-oklch(0.145 0 0) dark:bg-oklch(0.205 0 0) dark:text-oklch(0.985 0 0)",
        className,
      )}
      {...props}
    />
...
function CommandInput({
  ...
  return (
    <div data-slot="command-input-wrapper" className="p-1 pb-0">
      <InputGroup className="h-8! bg-oklch(0.922 0 0)/50 dark:bg-oklch(1 0 0 / 15%)/50">
...
function CommandSeparator({
  ...
  return (
    <CommandPrimitive.Separator
      ...
      className={cn("my-1 h-px bg-oklch(0.922 0 0)/50 dark:bg-oklch(1 0 0 / 10%)/50", className)}
      {...props}
    />
  );
}
