...
function FieldLabel({ className, ...props }: React.ComponentProps<typeof Label>) {
  return (
    <Label
      ...
      className={cn(
        "group/field-label peer/field-label flex w-fit gap-2 leading-snug group-data-[disabled=true]/field:opacity-50 has-data-checked:bg-oklch(0.922 0 0)/30 has-[>[data-slot=field]]:rounded-2xl has-[>[data-slot=field]]:border *:data-[slot=field]:p-4 dark:has-data-checked:bg-oklch(1 0 0 / 15%)/30",
...
function FieldDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-left text-sm leading-normal font-normal text-oklch(0.556 0 0) group-has-data-horizontal/field:text-balance [[data-variant=legend]+&]:-mt-1.5 dark:text-oklch(0.708 0 0)",
...
function FieldError({
  ...
  return (
    <div
      ...
      className={cn(
        "text-sm font-normal text-oklch(0.577 0.245 27.325) dark:text-oklch(0.704 0.191 22.216)",
        className,
      )}
      {...props}
    >
