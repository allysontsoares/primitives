...
// Input group has aria-labelledby; ViewControl groups do not
function getInputGroup() {
	return screen
		.getAllByRole("group")
		.find((g) => g.hasAttribute("aria-labelledby"))!;
}
...
describe("Input keyboard — digit entry", () => {
	...
	it("ArrowUp increments the focused segment", async () => {
		const user = userEvent.setup();
		render(
			<BasicPicker defaultValue={new Date(2024, 5, 15)} defaultOpen={false} />,
		);
		const segments = getSegments();
		const monthSeg = segments[0]!;
		await user.click(monthSeg);
	...
	it("ArrowUp on month 12 wraps to 1", async () => {
		const user = userEvent.setup();
		render(
			<BasicPicker defaultValue={new Date(2024, 11, 1)} defaultOpen={false} />,
		);
		const segments = getSegments();
		const monthSeg = segments[0]!;
		await user.click(monthSeg);
	...
	it("filling all segments dispatches date and calls onValueChange", async () => {
		...
		const [monthSeg, daySeg, yearSeg] = segments;
...
describe("Input ← Calendar sync", () => {
	it("clicking a calendar day updates the segments", async () => {
		...
		const day20 = cells.find(
			(c) =>
				c.textContent?.trim() === "20" && !c.hasAttribute("data-outside-month"),
		);
		await user.click(day20!);

		// Day segment should now show 20
		const daySegInput = segments.find(
			(s) => s.getAttribute("data-segment") === "day",
...
describe("Calendar navigation", () => {
	...
	it("ViewTrigger switches to month view", async () => {
		...
		const monthGrid = grids.find(
			(g) => g.getAttribute("aria-label") === "2024",
		);
...
describe("Day selection", () => {
	...
	it("closes calendar after single date selection (closeOnSelect default)", async () => {
		...
		const day10 = cells.find(
			(c) =>
				c.textContent?.trim() === "10" && !c.hasAttribute("data-outside-month"),
		);
...
describe("Year view navigation", () => {
	it("clicking ViewTrigger twice opens year view", async () => {
		...
		const grids = screen.getAllByRole("grid");
		const yearGrid = grids.find((g) =>
			/\d{4}.*\d{4}/.test(g.getAttribute("aria-label") ?? ""),
		);
